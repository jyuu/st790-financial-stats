...
for(j in 3:ncol(by_month)){ 
  temp <- by_month[,j] - by_month[,1]
  excess_return <- cbind(excess_return, temp)
  colnames(excess_return)[j-2] <- names(temp)
}
# get the alphas 
alphas <- c() 
betas <- c()
for(k in 1:ncol(excess_return)){
  results <- lsfit(by_month[,2], excess_return[,k])$coefficients
